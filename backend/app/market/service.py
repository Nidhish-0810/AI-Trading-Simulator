...
from app.market.schemas import MarketSummary, NewsItem, StockHistory, StockQuote, StockSearch

logger = logging.getLogger(__name__)
QUOTE_TTL, HISTORY_TTL, NEWS_TTL, SUMMARY_TTL, TRENDING_TTL = 60, 300, 600, 120, 90


async def fetch_quote(symbol: str, redis=None) -> Optional[StockQuote]:
...
async def fetch_history(symbol: str, period: str = "1y", interval: str = "1d", redis=None) -> Optional[StockHistory]:
...
async def fetch_news(symbol: str, redis=None) -> List[NewsItem]:
    ...
    if news and redis is not None:
        await set_cache(redis, cache_key, [n.model_dump() for n in news], ttl=NEWS_TTL)
    return news
...
async def fetch_trending(redis=None) -> List[StockQuote]:
    ...
    if redis is not None:
        await set_cache(redis, cache_key, [q.model_dump() for q in quotes], ttl=TRENDING_TTL)
    return quotes


async def fetch_movers(mover_type: str = "gainers", limit: int = 10, redis=None) -> List[StockQuote]:
...
async def fetch_multiple_quotes(symbols: List[str], redis=None) -> Dict[str, StockQuote]:
...
def get_all_stocks(page: int = 1, page_size: int = 20, search: Optional[str] = None, sector: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {"total": total, "page": page, "page_size": page_size,
            ...
            "stocks": [StockSearch(symbol=s["symbol"], name=s["name"], sector=s.get("sector", "")) for s in page_stocks]}
