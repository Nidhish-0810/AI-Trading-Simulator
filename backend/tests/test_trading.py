...
async def test_place_market_order_insufficient_funds(client: AsyncClient, auth_headers):
    response = await client.post("/api/trading/order", headers=auth_headers, json={"symbol": "AAPL", "order_type": "market", "side": "buy", "quantity": 1000000.0})
...
async def test_place_limit_order(client: AsyncClient, auth_headers):
    response = await client.post("/api/trading/order", headers=auth_headers, json={"symbol": "AAPL", "order_type": "limit", "side": "buy", "quantity": 10, "price": 100.0})
...
async def test_cancel_order(client: AsyncClient, auth_headers):
    order_res = await client.post("/api/trading/order", headers=auth_headers, json={"symbol": "MSFT", "order_type": "limit", "side": "buy", "quantity": 5, "price": 200.0})
